...
pub struct PaneState {
    ...
    /// Per-connected-host CPU and resident-memory history. Kept separate from
    /// agent rows so Top and Dash cannot accidentally mix populations.
    pub host_timeline: timeline::Ring,
...
pub fn spawn() -> Result<Server> {
    ...
    let state = Arc::new(Mutex::new(PaneState {
        ...
        host_timeline: timeline::Ring::new(Duration::from_secs(300), Duration::from_secs(1)),
...
fn sampler(state: Arc<Mutex<PaneState>>) {
    ...
    loop {
        ...
        let machines = fleet::group(readings);
        {
            ...
            // Every logical yggterm host gets its own history even when several
            // readings collapse to one physical Yggdrasil system. Top is a
            // host workbench; physical grouping belongs to the topology book.
            for machine in &machines {
                for reading in &machine.readings {
                    if reading["ok"].as_bool() != Some(true) {
                        continue;
                    }
                    let Some(host) = reading["host"].as_str() else {
                        continue;
                    };
                    let cores = reading["cpu_count"].as_f64().unwrap_or(1.0).max(1.0);
                    let cpu = reading["cpu_busy_pct"].as_f64().unwrap_or(0.0) / cores;
                    let total = reading["mem_total_kb"].as_i64().unwrap_or(0);
                    let available = reading["mem_available_kb"].as_i64().unwrap_or(0);
                    pane.host_timeline
                        .push(host, cpu, (total - available).max(0), 0);
                }
            }
            ...
            if let Some(m) = machines.iter().find(|m| m.reachable()) {
                ...
                if let Some(io) = p["zfs"]["iostat"].as_object() {
                    ...
                    let t_ms = std::time::SystemTime::now()
                        ...
                        .as_millis() as u64;
                    pane.zfs_history.push_back(ZfsIoSample {
                        t_ms,
                        read_bps: r,
                        write_bps: w,
                    });
        ...
        let hot = {
            ...
            pane.machines.iter().any(|m| {
                ...
                if let Some(tops) = p["top"].as_array() {
                    tops.iter().any(|t| {
                        t["comm"].as_str().unwrap_or("").contains("WebKitWebProces")
                            && t["cpu_pct"].as_f64().unwrap_or(0.0) > 45.0
                    })
                } else {
                    false
                }
...
fn handle_conn(stream: TcpStream, state: Arc<Mutex<PaneState>>) {
    ...
    match (method, path) {
        ...
        ("GET", "/pane/topo") => {
            let (view, machines, report, timeline, host_timeline, zfs_history) = {
                let pane = state.lock().unwrap();
                (
                    pane.view.clone(),
                    pane.machines.clone(),
                    pane.rows_report.clone(),
                    pane.timeline.clone(),
                    pane.host_timeline.clone(),
                    pane.zfs_history.clone(),
                )
            };
            respond(
                stream,
                200,
                &schema::viewport_view(
                    &view,
                    &machines,
                    &report,
                    &timeline,
        ...
        ("GET", "/pane/rail") => {
            ...
            respond(stream, 200, &schema::rail_view(&view, &machines, &report));
        }
        ("POST", "/action") => {
            ...
            if action == "mode" {
                if pane.view.select_mode(value) {
                    ...
                    pane.view.notice = Some("Unknown Ytop mode".to_string());
                }
            } else if let Some(pid) = action.strip_prefix("process_signal_menu:") {
                pane.view.process_signal_target = pid.parse::<i64>().ok();
                pane.view.notice = None;
                pane.touch();
            } else if action == "process_signal_cancel" {
                pane.view.process_signal_target = None;
                pane.touch();
            } else if let Some(rest) = action.strip_prefix("process_signal:") {
                let Some((signal, pid)) = rest.split_once(':') else {
                    pane.view.notice = Some("Invalid process signal action".to_string());
                    pane.touch();
                    respond(stream, 200, &json!({"ok": false}));
                    return;
                };
                let Ok(pid) = pid.parse::<i64>() else {
                    pane.view.notice = Some("Invalid process id".to_string());
                    pane.touch();
                    respond(stream, 200, &json!({"ok": false}));
                    return;
                };
                let signal = signal.to_string();
                let host = pane.view.selected_host.clone();
                pane.view.process_signal_target = None;
                pane.view.notice = Some(format!("Sending SIG{signal} to PID {pid}…"));
                pane.touch();
                drop(pane);
                ...
                std::thread::spawn(move || {
                    let remote = (host != fleet::LOCAL).then_some(host.as_str());
                    let result = probe::send_signal(remote, pid, &signal);
                    let mut pane = signal_state.lock().unwrap();
                    pane.view.notice = Some(match result {
                        Ok(()) => format!("Sent SIG{signal} to PID {pid}"),
                        Err(error) => format!("Could not signal PID {pid}: {error}"),
                    });
                    pane.touch();
                });
                return;
                ...
                let alias = body
                    .get("values")
                    .and_then(|v| v.get("new_machine_alias"))
                    .and_then(Value::as_str)
                    .unwrap_or("");
                let label = body
                    .get("values")
                    .and_then(|v| v.get("new_machine_label"))
                    .and_then(Value::as_str)
                    .unwrap_or("");
                let is_ygg = body
                    .get("values")
                    .and_then(|v| v.get("new_machine_yggdrasil"))
                ...
                match rows::clean_jankbox_on_dev() {
                    Ok(killed) => {
                        pane.view.notice =
                            Some(format!("🧹 Cleaned {killed} leaked/twin processes!"));
                    }
                    Err(e) => {
                ...
                let mode = if action.ends_with("_dash") {
                    "dash"
                } else {
                    "top"
                };
                ...
                let desc = body
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or("Composed via ytop skill")
                    .to_string();
                let author = body
                    .get("payload")
                ...
                let pages_raw = body
                    ...
                    .cloned()
                    .unwrap_or_default();
                let pages: Vec<crate::notebook::Page> = pages_raw
                    .into_iter()
                    .filter_map(|pv| {
                        ...
                        let markdown = pv
                            ...
                            .to_string();
                        if let Ok(page) =
                            serde_json::from_value::<crate::notebook::Page>(pv.clone())
                        {
                            return Some(page);
                        }
                        ...
                        let ytrace_queries = pv
                            .get("ytrace_queries")
                            .and_then(|v| {
                                serde_json::from_value::<Vec<crate::notebook::YtraceQuery>>(
                                    v.clone(),
                                )
                                .ok()
                            })
                ...
                let nb = crate::notebook::Notebook {
                    ...
                    pages: if pages.is_empty() {
                        vec![crate::notebook::Page {
                            ...
                            markdown: format!(
                                "# {title}\n\nComposed via ytop skill on host `{}`.",
                                pane.view.selected_host
                            ),
                            ytrace_queries: vec![],
                            chart: None,
                            live: None,
...
pub fn print_notebook(id: &str, page: Option<usize>) -> Result<()> {
    ...
    if p.has_ytrace() {
        let qs: Vec<String> = p
            ...
            .map(|q| {
                format!(
                    "ytrace query --app {} --category {} --name {} --since {}s",
                    q.provider,
                    q.category,
                    q.name,
                    q.since_ms / 1000
...
pub fn print_once(mode: &str, tab: &str, as_json: bool) -> Result<()> {
    if mode == schema::MODE_DASH {
        let report = rows::scan_all_hosts();
        let (conditions, total_records) = crate::complaints::read_live("yggterm", COMPLAINT_WINDOW);
        if as_json {
            println!(
                "{}",
        ...
        if tab == "jankbox" {
            println!(
                "{}",
                crate::complaints::render(&conditions, total_records, COMPLAINT_WINDOW)
            );
        ...
        if tab == "supervision" {
            ...
            for r in &report.rows {
                by_state
                    .entry(r.supervision_state.as_str())
                    .or_default()
                    .push(&r.seat);
            }
        ...
        println!(
            "  Y T O P   ·   F L E E T   A G E N T   R O W S   &   J A N K B O X   D A S H B O A R D"
        );
        println!(
            "=========================================================================================================="
        );
        ...
        println!("  {}", "=".repeat(110));
        println!(
            "  Total Seats: {}  ·  Live: {}  ·  Agent CPU: {:.1}%  ·  Agent RAM: {:.1} MB  ·  Context: {:.1} MB",
            report.total_rows,
            report.live_count,
        ...
        println!(
            "  Jankbox Leaks: {} spinning subshells  ·  Twin Duplicate Alarms: {}",
    ...
    for machine in machines {
        ...
        for container in principal["containers"]
            .as_array()
            .cloned()
            .unwrap_or_default()
        {
...
mod tests {
    ...
    fn action_value_reads_the_libyggterm_values_envelope() {
        let body = json!({"action": "mode", "values": {"value": "dash"}});
        assert_eq!(action_value(&body), "dash");
    }

    #[test]
    fn action_value_keeps_legacy_top_level_compatibility() {
        let body = json!({"action": "mode", "value": "top"});
        assert_eq!(action_value(&body), "top");
    }
}
